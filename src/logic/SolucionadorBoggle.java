...
public class SolucionadorBoggle {
    ...
    public SolucionadorBoggle(LinkedGraph graph) {
        this.graph = graph;
    ...
    private static char[] extractLetters(LinkedGraph g) {
        int n = TableroBoggle.ROWS * TableroBoggle.COLS; // 16
        char[] arr = new char[n];
        for (int i = 0; i < n; i++) {
            Vertex v = (Vertex) g.getVerticesList().get(i);
            CeldaTablero cell = (CeldaTablero) v.getInfo();
            arr[i] = cell.letter;
        }
        return arr;
    }
    ...
    public Set<String> findAllWords(Diccionario dict, int minLen, java.util.function.Predicate<String> accept) {
    ...
    private void dfsEnumerate(int idx, Diccionario dict, int minLen, boolean[] visited, StringBuilder sb,
    ...
    public Set<String> findAllWordsFiltered(Diccionario dict, int minLen) {
